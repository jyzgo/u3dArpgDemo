...
public static class PhotonNetwork
{
    ...
    public const string versionPUN = "1.20g";
    ...
    public static string currentLobbyName { get { return networkingPeer.mCurrentLobbyName; } }

    public static LobbyType currentLobbyType { get { return networkingPeer.mCurrentLobbyType; } }
    ...
    public static bool OpJoinLobby(string name, LobbyType lobbyType)
    {
        bool sent = networkingPeer.OpJoinLobby(name, lobbyType);
        if (sent)
        {
            networkingPeer.mCurrentLobbyType = lobbyType;
            networkingPeer.mCurrentLobbyName = name;
        }

        return sent;
    }

    public static bool OpLeaveLobby()
    {
        bool sent = networkingPeer.OpLeaveLobby();
        if (sent)
        {
            networkingPeer.mCurrentLobbyType = LobbyType.Default;
            networkingPeer.mCurrentLobbyName = null;
        }
    ...
    {
        CreateRoom(roomName, true, true, 0, null, null, null, LobbyType.Default);
    }
    ...
    {
        CreateRoom(roomName, isVisible, isOpen, maxPlayers, null, null, null, LobbyType.Default);
    }
    ...
    /// PhotonNetwork.autoCleanUpPlayerObjects will become this room's AutoCleanUp property and that's used by all clients that join this room.
    /// 
    /// Room names are unique. They will be attached to the specified lobby (by name and type). Use null as lobbyName and the room belongs to 
    /// the default lobby.
    /// Multiple lobbies can help separate players by map or skill or game type. Each room can only be found in one lobby (no matter if defined 
    /// by name and type or as default).
    /// </remarks>
    /// <param name="roomName">Unique name of the room to create. Pass null or "" to make the server generate a name.</param>
    /// <param name="isVisible">Shows (or hides) this room from the lobby's listing of rooms.</param>
    /// <param name="isOpen">Allows (or disallows) others to join this room.</param>
    ...
    public static void CreateRoom(string roomName, bool isVisible, bool isOpen, int maxPlayers, Hashtable customRoomProperties, string[] propsToListInLobby, string lobbyName, LobbyType lobbyType)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                networkingPeer.mCurrentLobbyName = lobbyName; 
                networkingPeer.mCurrentLobbyType = lobbyType;
                networkingPeer.OpCreateGame(roomName, isVisible, isOpen, (byte)maxPlayers, autoCleanUpPlayerObjects, customRoomProperties, propsToListInLobby, lobbyName, lobbyType);
            }
        }
    }
    ...
    {
        JoinRandomRoom(null, 0, MatchmakingMode.FillRoom, null, LobbyType.Default, null);
    }
    ...
    {
        JoinRandomRoom(expectedCustomRoomProperties, expectedMaxPlayers, MatchmakingMode.FillRoom, null, LobbyType.Default, null);
    }
    ...
    {
        JoinRandomRoom(expectedCustomRoomProperties, expectedMaxPlayers, matchingType, null, LobbyType.Default, null);
    }

    /// <summary>
    /// Attempts to join an open room, filtered by provided sqlLobbyFilter. Simply fails if none is currently available.
    /// </summary>
    /// <remarks>
    /// If this fails, you can still create a room (and make this available for the next who uses JoinRandomRoom).
    /// Alternatively, try again in a moment or relax your filtering.
    /// 
    /// The sqlLobbyFilter can be used to match certain room properties. These must be set per room.
    /// To make room properties available in the lobby (for filtering), they also have to be in the string[] list of propertiesInLobby.
    /// See: CreateRoom
    /// </remarks>
    /// <param name="lobbyName">Name of the lobby in which to find a room. Default: null. If this is null, the lobbyType is ignored and sent as Default, too.</param>
    /// <param name="lobbyType">Type of the (named)lobby this game gets added to.</param>
    ...
    public static void JoinRandomRoom(string lobbyName, LobbyType lobbyType, string sqlLobbyFilter)
    {
        JoinRandomRoom(null, 0, MatchmakingMode.FillRoom, lobbyName, lobbyType, sqlLobbyFilter);
    }
    ...
    /// <param name="lobbyType">Type of the (named)lobby this game gets added to.</param>
    /// <param name="sqlLobbyFilter">Basically the "where" clause of a sql statement. Use null for random game. Examples: "C0 = 1 AND C2 > 50". "C5 = \"Map2\" AND C2 > 10 AND C2 < 20"</param>
    public static void JoinRandomRoom(Hashtable expectedCustomRoomProperties, byte expectedMaxPlayers, MatchmakingMode matchingType, string lobbyName, LobbyType lobbyType, string sqlLobbyFilter)
    {
        if (connectionStateDetailed == PeerState.Joining || connectionStateDetailed == PeerState.Joined || connectionStateDetailed == PeerState.ConnectedToGameserver)
        {
        ...
        else
        {
            ...
            networkingPeer.OpJoinRandomRoom(expectedRoomProperties, 0, null, matchingType, lobbyName, lobbyType, sqlLobbyFilter);
        }
    }
