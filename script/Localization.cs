...
public class Localization : MonoBehaviour
{
    ...
    private const string k_language_asset_path = "Assets/Data/Localization/NGUI/{0}.txt";

    private const string k_language_resources_path = "SplashText/NGUI/{0}";
    ...
    public EnumResourcePath Mode { get; set; }
		...
		set
		{
			if (mLanguage != value)
			{
				...
				if (!string.IsNullOrEmpty(value))
				{
                    ...
                    TextAsset txt;
                    if (Mode == EnumResourcePath.language_bundle)
                    {
                        txt = InJoy.AssetBundles.AssetBundles.Load(string.Format(k_language_asset_path, value)) as TextAsset;
                    }
                    else
                    {
                        string path = string.Format(k_language_resources_path, value);
                        txt = Resources.Load(path, typeof(TextAsset)) as TextAsset;
                    }
                    //Debug.LogError("Load from resources folder! Language = " + value);
	...
	void Awake ()
	{
		if (mInstance == null)
		{
            ...
            Mode = EnumResourcePath.resources;
			Init();
		}
	...
	public void Init()
	{
		currentLanguage = LocalizationContainer.CurSystemLang;
...
#elif UNITY_ANDROID
		if (mDictionary.TryGetValue(key + "_ANDROID", out val)) return val;
#elif UNITY_STANDALONE_OSX
		if (mDictionary.TryGetValue(key + "_MACOS", out val)) return val;
#endif
