...
public class AttackSkull : AttackBase
{
    public FC_CHARACTER_EFFECT attackEffect = FC_CHARACTER_EFFECT.INVALID;
    ...
    private void UpdataEffect()
    {
        float currentAnimPercent = _owner.ACOwner.AniGetAnimationNormalizedTime();

        if (!_isPlaying)
        {
            if (currentAnimPercent > startPercent)
            {
                CharacterEffectManager.Instance.PlayEffect(attackEffect, _owner.ACOwner._avatarController, -1);
                _isPlaying = true;
            }
        }
    }
