...
/// </summary>
namespace InJoy.Utils
{
    public class JsonHashtable : Hashtable
    {
	...
	public class FCJson
	{
		...
		public static object jsonDecode( string json )
		{
			...
			if( json != null )
			{
				...
				int index = 0;
				bool success = true;
				object value = FCJson.parseValue( charArray, ref index, ref success );
	
				if( success )
					FCJson.lastErrorIndex = -1;
				else
					FCJson.lastErrorIndex = index;
		...
		public static string jsonEncode( object json )
		{
			...
			var success = FCJson.serializeValue( json, builder );
		...
		{
			return ( FCJson.lastErrorIndex == -1 );
		}
		...
		{
			if( FCJson.lastErrorIndex == -1 )
			{
				return "";
			}
			else
			{
				int startIndex = FCJson.lastErrorIndex - 5;
				int endIndex = FCJson.lastErrorIndex + 15;
				if( startIndex < 0 )
					startIndex = 0;
	
				if( endIndex >= FCJson.lastDecode.Length )
					endIndex = FCJson.lastDecode.Length - 1;
	
				return FCJson.lastDecode.Substring( startIndex, endIndex - startIndex + 1 );
			}
		}
		...
		protected static Hashtable parseObject( char[] json, ref int index )
		{
			...
			while( !done )
			{
				...
				if( token == FCJson.TOKEN_NONE )
				{
					return null;
				}
				else if( token == FCJson.TOKEN_COMMA )
				{
					nextToken( json, ref index );
				}
				else if( token == FCJson.TOKEN_CURLY_CLOSE )
				{
					nextToken( json, ref index );
					return table;
				}
		...
		protected static ArrayList parseArray( char[] json, ref int index )
		{
			...
			while( !done )
			{
				...
				else if( token == FCJson.TOKEN_SQUARED_CLOSE )
				{
					nextToken( json, ref index );
					break;
				}
		...
		protected static object parseValue( char[] json, ref int index, ref bool success )
		{
			...
			{
				case FCJson.TOKEN_STRING:
					return parseString( json, ref index );
				case FCJson.TOKEN_NUMBER:
					return parseNumber( json, ref index );
				case FCJson.TOKEN_CURLY_OPEN:
					return parseObject( json, ref index );
				case FCJson.TOKEN_SQUARED_OPEN:
					return parseArray( json, ref index );
				case FCJson.TOKEN_TRUE:
					nextToken( json, ref index );
					return Boolean.Parse( "TRUE" );
				case FCJson.TOKEN_FALSE:
					nextToken( json, ref index );
					return Boolean.Parse( "FALSE" );
				case FCJson.TOKEN_NULL:
					nextToken( json, ref index );
					return null;
				case FCJson.TOKEN_NONE:
					break;
			}
	...
	s += Char.ConvertFromUtf32((int)codePoint);
		...
		protected static object parseNumber( char[] json, ref int index )
		{
			...
			index = lastIndex + 1;
			string numStr = new string( numberCharArray );
			if( numStr.Contains( "." ) )
		...
		protected static int nextToken( char[] json, ref int index )
		{
			...
			{
				case '{':
					return FCJson.TOKEN_CURLY_OPEN;
				case '}':
					return FCJson.TOKEN_CURLY_CLOSE;
				case '[':
					return FCJson.TOKEN_SQUARED_OPEN;
				case ']':
					return FCJson.TOKEN_SQUARED_CLOSE;
				case ',':
					return FCJson.TOKEN_COMMA;
				case '"':
					return FCJson.TOKEN_STRING;
				case '0':
				case '1':
				case '2':
				case '3':
				...
				case '9':
				case '-': 
					return FCJson.TOKEN_NUMBER;
				case ':':
					return FCJson.TOKEN_COLON;
			}
			index--;
